...
export default async function CategoryTree({ path, route, searchParams }: Props) {
    const shimmer = (
        <section className="mt-8">
            <p className="text-sm opacity-50 mb-4">
                <span className="mb-4 h-4 w-24 animate-pulse rounded bg-mist/20" />
            </p>
    ...
    return (
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <Suspense fallback={shimmer}>
                <Products path={path} route={route} searchParams={searchParams} />
            </Suspense>
        </div>
    )
}
